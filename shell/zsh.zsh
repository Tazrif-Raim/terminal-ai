# shell/zsh.zsh - Terminal AI Zsh wrapper
# Source this from your ~/.zshrc:
#   source /path/to/terminal-ai/shell/zsh.zsh

_TERMINAL_AI_ROOT="$(cd "$(dirname "$0")/.." && pwd 2>/dev/null)"
...
_ai_set_env() {
    ...
    export TERMINAL_AI_SHELL_NAME="zsh"
    export TERMINAL_AI_SHELL_VERSION="${ZSH_VERSION:-}"
...
ai() {
    ...
    if [[ "$agent_logs_mode" == true ]]; then
        ...
        local open_arg=""
        [[ ${#prompt_args[@]} -gt 0 && "${prompt_args[1]}" == "open" ]] && open_arg="open"
